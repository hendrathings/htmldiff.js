var Match, calculate_operations, consecutive_where, create_index, diff, find_match, find_matching_blocks, html_to_tokens, is_insert_of_tag, is_delete_of_tag, is_end_of_tag, is_start_of_tag, is_tag, is_whitespace, isnt_tag, op_map, recursively_find_matching_blocks, render_operations, wrap;

is_delete_of_tag = function(str) {
  return str.includes("<del>");
};

is_insert_of_tag = function(str) {
  return str.includes("<ins>");
};
...
op_map.replace = function(op, before_tokens, after_tokens) {
  const delText = (op_map["delete"](op, before_tokens, after_tokens));
  const insText = (op_map.insert(op, before_tokens, after_tokens));
  let res = delText + insText;

  if (!is_delete_of_tag(delText) && !is_insert_of_tag(insText)) {
    res = insText
  }
